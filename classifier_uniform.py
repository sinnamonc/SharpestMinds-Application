...
for i in np.arange(1, 8):
    ...
    train_df = pd.concat([train_df, data_temp[
        :train_df_size_per_index]], axis=0, join='outer')
    # Put the next valid_df_size into the validation set
    valid_df = pd.concat([
        valid_df, data_temp[train_df_size_per_index:
                            train_df_size_per_index+valid_df_size_per_index]
                            ], axis=0)
    # Put the remainder into the training set
    test_df = pd.concat([
        test_df, data_temp[train_df_size_per_index+valid_df_size_per_index:]
        ], axis=0)
...
# train the model several epochs, and test on the test set.
...
for _ in np.arange(10000):
    ...
    valid_loss.append(model.test_on_batch(valid_data, valid_labels,
                                          sample_weight=None))
