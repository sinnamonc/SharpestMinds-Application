...
def import_data(mean_zero=False):
    """Imports the data from file and returns the a dataframe,
    df, containing the data, scaled so that each entry is between 0 and 1.
    """
...
def get_data_sets(mean_zero=False, seed=None):
    ...
    Recommended usage: test_data, test_labels, valid_data, valid_labels,
    train_data, train_labels = get_data_sets()
    """
    ...
    valid_df = df[train_df_size_per_index:
                  train_df_size_per_index+valid_df_size_per_index]
...
#     shuffle_in_unison(train_data, train_labels)
    ...
    return test_data, test_labels, valid_data, valid_labels,
    train_data, train_labels

# def augment_data(data, labels):
#     """This function returns an augmented data set where noise has been
#     added to to the first 10 fields to create a larger collection of data.
#     """
